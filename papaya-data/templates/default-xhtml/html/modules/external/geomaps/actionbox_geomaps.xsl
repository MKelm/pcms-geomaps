...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/1999/xhtml">
  ...
  <xsl:template name="include-js-files">
    <!-- map js files -->
    <xsl:choose>
      <xsl:when test="base/api/@type = 'ol'">
        <!-- open layers -->
        <script type="text/javascript" src="http://openlayers.org/api/OpenLayers.js"><xsl:comment><xsl:text> </xsl:text></xsl:comment></script>
        <script type="text/javascript" src="{base/@scripts-path}geomaps-di/ol.js"><xsl:comment><xsl:text> </xsl:text></xsl:comment></script>
      </xsl:when>
    ...
    <xsl:if test="markers and markers/@mode != 'hidden'">
      ...
      <xsl:if test="markers">
        <xsl:choose>
          ...
          <xsl:when test="markers/data-page/@url != ''">
            <xsl:choose>
